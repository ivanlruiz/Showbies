# Textos de la ficha de Play

Play pide estos tres textos por idioma. Los límites son de Google y están contados.

---

## Inglés (idioma principal)

### Nombre de la app (máx. 30)
```
ShowBies: Zombie Shooter
```
(24 caracteres. "ShowBies" solo también sirve, pero sumar dos palabras de lo que es el juego ayuda a que
aparezca en las búsquedas.)

### Descripción corta (máx. 80)
```
Twin-stick zombie shooter. Survive the horde, grab the coins, upgrade, repeat.
```
(77 caracteres.)

### Descripción larga (máx. 4000)
```
Zombies are coming. All of them. At once.

ShowBies is a fast twin-stick shooter: move with one thumb, aim and shoot with the other, and throw grenades
when things get out of hand. Every zombie you drop spills coins — and every coin buys the upgrades that decide
how long you last next time.

SURVIVE TWO WAYS
• Wave mode: waves that end when the last zombie drops. Every wave is bigger, and every tenth one sends a boss.
• Endless mode: no waves, no mercy. The longer you stay alive, the tougher they get.

UPGRADE BETWEEN RUNS
Damage, fire rate, max health, pickup magnet, loot. You start weak on purpose: the first run should buy you
your first upgrade, and that upgrade should show up in the second run.

UNLEASH THE RAGE
Buy it once and you get a button on the HUD: double fire rate, double damage and a speed boost for six seconds,
every two minutes. Use it when you're surrounded — it's exactly what it's for.

FIVE KINDS OF DEAD
The plain one, the fast one, the very fast one, the tank that eats a whole magazine, and the boss with 500 HP.
They all hit harder as the waves go up.

• Plays in landscape, with touch controls made for thumbs
• No internet needed — everything runs on your phone
• No accounts, no sign-ups, nothing to fill in
• Free, and it doesn't collect your data
```

---

## Español de España (es-ES)

Cargado así en Play. La ficha de Latinoamérica (es-419) usa el mismo texto con "junta monedas",
"Empiezas débil", "tiene que alcanzarte" y "celular" en vez de "recoge", "flojo", "darte" y "móvil".

### Nombre de la app
```
ShowBies: Matazombis
```

### Descripción corta
```
Shooter de zombis a dos palancas. Sobrevive, recoge monedas, mejora y repite.
```

### Descripción larga
```
Vienen los zombis. Todos. Al mismo tiempo.

ShowBies es un shooter a dos palancas rápido: te mueves con un pulgar, apuntas y disparas con el otro, y lanzas granadas
cuando la cosa se descontrola. Cada zombi que cae suelta monedas, y cada moneda compra las mejoras que deciden
cuánto aguantas la próxima vez.

DOS FORMAS DE SOBREVIVIR
• Modo oleadas: cada oleada termina cuando cae el último zombi. Cada una es más grande, y cada diez viene un jefe.
• Modo libre: sin oleadas y sin piedad. Cuanto más aguantas, más duros se vuelven.

MEJORA ENTRE PARTIDAS
Daño, cadencia, vida máxima, imán de monedas, botín. Empiezas flojo a propósito: la primera partida tiene que
darte para la primera mejora, y esa mejora tiene que notarse en la segunda.

DESATA LA FURIA
Se compra una sola vez y te deja un botón en la pantalla: cadencia y daño al doble, más velocidad, seis
segundos, cada dos minutos. Guárdala para cuando estés rodeado, que es justo para eso.

CINCO TIPOS DE ZOMBI
El normal, el rápido, el rapidísimo, el tanque que se come un cargador entero y el jefe de 500 de vida. Todos
pegan más fuerte a medida que suben las oleadas.

• Se juega en horizontal, con controles táctiles pensados para los pulgares
• No necesita internet: todo funciona en tu móvil
• Sin cuentas ni registros
• Gratis, y no recopila tus datos
```

---

## Datos de contacto (van en la ficha, son públicos)

- **Email de contacto**: obligatorio. Elegí uno que no te moleste tener público; muchos desarrolladores
  crean uno aparte para esto.
- **Sitio web**: opcional. Si publicás la política de privacidad en GitHub Pages, podés usar esa misma URL.

## Gráficos que hay que subir

| Qué | Tamaño | Notas |
|---|---|---|
| Icono | 512 x 512 PNG | El que ya tiene la app, exportado en ese tamaño |
| Gráfico de funciones | 1024 x 500 | El banner de arriba de la ficha; no puede tener texto cortado |
| Capturas de teléfono | mínimo 2, 16:9 horizontal | Una del juego con zombis encima y otra de la tienda de mejoras |

Para las capturas, las mejores son: **el juego en plena horda** (se ve el HUD, las monedas y la vida en rojo),
**la tienda de mejoras** (se ve que es un incremental) y **la pantalla de derrota** con las monedas ganadas.
